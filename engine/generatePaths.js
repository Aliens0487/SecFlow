...
listener.on('activity.end', (api) => {
  ...
  if (isExecutable(api)) {
    ...
    if (!wasStartLast()) {
      executeSecFlow(currentState, newState,lastActivityIndex);
    }
    if (index != -1) {
...
async function compileSecFlow() {
  await executeWithPromise('../bin/secFlow compile ' + path).then((stdout) => {
    console.log(stdout);
  });
}

async function fillInputs(stateId) {
  await executeWithPromise(`../bin/secFlow fill-inputs state-${stateId}.json ../key.json`).then((stdout) => {
    console.log(stdout);
  });
}

async function createWitness(stateId) {
  await executeWithPromise(`../bin/secFlow witness ${path} state-${stateId}.json ../key.json --full full-${stateId}.wtns`).then((stdout) => {
    console.log(stdout);
  });
}

async function proveSecFlow(stateId) {
  await executeWithPromise(`../bin/secFlow prove circuit.r1cs pk.bin full-${stateId}.wtns -o proof-${stateId}`).then((stdout) => {
    //console.log(stdout);
  });
}

async function setupSecFlow() {
  // ./bin/zkfw setup circuit.r1cs
  await executeWithPromise('../bin/secFlow setup circuit.r1cs').then((stdout) => {
    console.log(stdout);
  });
}
...
async function executeSecFlow(currentState, newState,completedId) {
  ...
  // Run the secFlow fill inputs ("../bin/secFlow fill-inputs state.json ../key.json")
